...
def single():
    ...
    domain_parts = single_domain.split(':')
    if len(domain_parts) > 1:
        domain_to_ip = domain_parts[0]
        port_number = domain_parts[1]
    ...
    while ans:
      ...
      """)
        ...
        if ans == "1":
            ...
            os.system('cls' if os.name == 'nt' else 'clear')
            print(Colorate.Vertical(Colors.green_to_yellow, banner, 2))
            print(Colors.red+"\n [+] Thanks For Using Web-Scan! See You Tomorrow")
            ans = None
        else:
